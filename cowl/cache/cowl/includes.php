...
// Constant: <COWL_BASE>
// The root of the URL. Will almost always be '/'.
	...
	*/
	...
	public function parse()
	{
		...
		if ( ! count($pieces) || empty($pieces[0]) )
		{
			...
			$return = array($this->default_command);
		}
		else
		{
			$directories = $pieces;
			...
			if ( ! $directories || ! count($directories) )
			{
				...
				else
				{
					...
					$return = array_merge(array($this->error_command), $pieces);
				}
			}
			else
			{
		...
		$return[-3] = $directory;
		$return[-2] = $command_name;
		$return[-1] = $response_type;
...
abstract class Command
{
	...
	*/
	...
	public final function run($argv)
	{
		...
		// Set response type shell
		$this->template->setType($argv[-1]);
...
class RegistryMemberNotFoundException extends Exception {}
...
class RequestInfoNotFoundException extends Exception {}
