	...
	*/
	...
	public function parse()
	{
		...
		else
		{
			...
			$was_main = is_dir(self::$commands_dir . $glued . DIRECTORY_SEPARATOR . 'main');
			if ( $was_main )
			{
				$directories[] = 'main';
			}
...
$tests = array(
	'' => array('MainCommand'),
	'path/to/nothing' => array('ErrorCommand', 'path', 'to', 'nothing'),
...
foreach ( $tests as $url => $test )
{
	...
	if ( $result != $test )
	{
		printf('<strong>"%s" failed.' . PHP_EOL . 'Expected: 	%s' . PHP_EOL . 'Found: 		%s</strong>' . PHP_EOL . PHP_EOL, $url, implode(', ', $test), implode(', ', $result));
	}
	else
		$passed .= '.';
}
